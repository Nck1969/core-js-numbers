...
function getLastDigit(value) {
  return value < 10 ? value : value % (10 * (value.toString().length - 1));
}
...
function getFibonacciNumber(index) {
  function getNumber(indexOfNumber) {
    const numbers = [0, 1];

    while (numbers.length - 1 !== indexOfNumber) {
      numbers.push(numbers.at(-1) + numbers.at(-2));
    }

    return numbers[indexOfNumber];
  }
...
function getSumToN(n) {
  return n - 1 > 0 ? n + getSumToN(n - 1) : n;
}
